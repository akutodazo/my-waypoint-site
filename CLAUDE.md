...
- **全体撮影モード**: 前方・横オーバーラップがどちらも0のとき発動（小麦等の色ムラ確認用）。範囲全体が写る高度を自動計算（上限149m・最大3枚・余裕5%）し、2〜3点で撮影。DJIのウェイポイント飛行は最小2点のため「1枚」は不可（1枚判定でも2点=2枚）。3枚で収まらない大圃場はエラー案内。カバレッジはヘディング非依存の近似→実機確認が必要（3-7）
