...
- 撮影プリセット（検証済みパラメータ）: 高度20m/-90°、10m/-90°、10m/-60°。＋全体撮影（色確認、0/0）
- **全体撮影モード**: 前方・横オーバーラップがどちらも0のとき発動（小麦等の色ムラ確認用）。範囲全体が写る高度を自動計算（上限149m・余裕5%）し、まず1〜3枚を試行。収まらない大圃場は**高度149m固定で必要枚数（4枚、5枚…）にジグザグ順で自動分割**（エラーにしない）。DJIのウェイポイント飛行は最小2点のため「1枚」は不可（1枚判定でも2点=2枚）。カバレッジはヘディング非依存の近似→実機確認が必要（3-7）
...
| 3-7  | 実地ドッグフーディング（ジンバル角・各点撮影・点数上限・**全体撮影モードのカバレッジ**の実機確認、/guideとREADMEに環境記入） | ⬜   |
